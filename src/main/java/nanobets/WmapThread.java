...
public class WmapThread  extends Thread {

	/**
	 * Runs withdrawMap removal thread
	 */
	public void run() {
				System.out.println("Withdraw map removal thread started");
				try {
					if(withdrawMap.isEmpty() == false) {
						for(Map.Entry<String, Long> entry : withdrawMap.entrySet()) {
							long time = entry.getValue();	
							String id = entry.getKey();
							//check if wmap entry has been in the wmap for over 5 minutes
							...
							if(current_time > check_against) {
								//go ahead and delete wmap entry
								withdrawMap.remove(id);
							}
						}
					}
				}
